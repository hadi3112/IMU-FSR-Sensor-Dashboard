...
export function TelemetryStrip() {
  ...
  return (
    <div ref={rootRef} className="relative shrink-0">
      <div className="mb-[5px] grid grid-cols-4 divide-x divide-transparent">
        {metrics.map((s) => (
          <div key={s.key} className="flex flex-col items-center px-1 text-center">
            <div className="text-[12px] font-semibold uppercase leading-none tracking-wider text-[#8e8e93]">{s.label}</div>
          </div>
        ))}
      </div>

      <div className="rounded-2xl border border-white/10 bg-gradient-to-br from-[#1b1b20] via-[#151518] to-[#0f0f12] p-2 shadow-[0_12px_48px_rgba(0,0,0,0.55)] ring-1 ring-white/5">
        <div className="grid grid-cols-4 divide-x divide-white/10">
          {metrics.map((s) => (
            <div key={s.key} className="flex flex-col items-center justify-center gap-0.5 px-1 py-1 text-center">
              {s.icon}
